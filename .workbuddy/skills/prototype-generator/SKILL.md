...
### 3. 1/3 屏幕宽侧边面板布局约束 (1/3 Screen Width Side Panel Layout)
- **1/3 宽度侧边抽屉**：所有的设置面板必须设计为“侧边抽屉（Side Drawer / Overlay Panel）”样式，固定靠左侧对齐，右侧的 2/3 空间完全留白，用于展示底层的视频流。
- **高度铺满与贴边对齐**：侧边栏面板必须**高度垂直铺满整个屏幕（即 `height: 100vh` 或 `1080px`，`top: 0; bottom: 0;`）**，并且牢牢**贴合在屏幕左边缘（固定靠左对齐，如 `left: 0;`）**。**严禁**生成任何上下有边距、悬浮在屏幕中央、形似卡片盒子的独立悬浮弹窗，也**严禁**靠右侧贴边。它必须像真实电视设置菜单一样靠左贴边且贯穿整个垂直高度。
- **宽度度量限制**：主面板在 1920x1080 原型分辨率下，宽度必须严格限定为屏幕宽度的 **1/3**（即固定为 `640px`，或在 React 布局中使用 Tailwind 的 `w-1/3` 与 `width: 33.333%`）。所有级联菜单的滑动切换和展示必须在此 `640px` 宽度的边界内运行，实现精致的侧边栏 TV 设置体验。
