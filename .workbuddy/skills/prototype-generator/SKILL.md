...
### 3. 区域化布局与侧边栏对齐约束 (Regional Layout & Side Panel Constraints)

根据生成原型所处的目标区域（国内/海外）及操作系统规范，面板布局结构必须进行严格的区域化隔离，禁止一刀切地采用同一种布局：

- **【中国区 (CN - TCL OS)】左置控制中心与左置 1/3 级联设置**：
  - **控制中心（快捷设置）**：固定靠**左侧**对齐（抽屉式滑出，高度垂直铺满屏幕 `height: 100vh` 或 `1080px`，宽度约 `420px`，贴合屏幕左边缘 `left: 0`）。这是用户按遥控器设置键最先弹出的主面板。
  - **系统设置菜单（如点击“全部设置”进入的界面）**：必须继续在屏幕**左侧**展示（高度垂直铺满整个屏幕，整体靠左贴边对齐，宽度严格限制在屏幕的 1/3，即固定为 `640px`）。
    - **内部结构**：在 `640px` 宽的左侧设置面板内，左侧为窄条状的垂直一级分类图标栏（宽约 `80px`），右侧为当前选中的二级设置项列表（宽约 `560px`）。
    - **高保真矢量图标（Inline SVGs）**：分类图标栏中必须使用高精度的 Inline SVG 图标，绝对禁止使用 Emoji 字符。AI 助手生成或调整项目时，必须包含以下矢量图标定义：
      - *图像 (Image/Monitor)*: 宽屏带底座显示器图标；
      - *声音 (Sound/Speaker)*: 扬声器加双重声波图；
      - *网络 (Wifi)*: 渐宽的三层无线波束；
      - *蓝牙 (Bluetooth)*: 标准蓝牙双折线符号；
      - *信号源 (Source/Input)*: 双插头对接线；
      - *AI智能 (AI/Brain)*: 双侧大脑瓣叶轮廓；
      - *通用 (General/Wrench)*: 45度倾斜扳手工具；
      - *个性化 (Personalization/Sliders)*: 带调节节点的滑块轨道；
      - *关于 (About/Info)*: 带圆环的字母 "i" 信息标识。
    - **右侧留白**：右侧的 2/3 空间完全留白，用于展示底层的视频流。**严禁**生成任何跨出 1/3 边界的宽屏面板，也**严禁**生成上下有边距的独立悬浮卡片。

- **【海外区 (Overseas - Google TV / Fire TV)】右置控制中心与左置系统设置**：
  - **控制中心（快捷设置）**：用户按遥控器设置键最先弹出的主面板。必须固定靠**右侧**贴边对齐（高度垂直铺满屏幕 `height: 100vh`，宽度约 `400px`，贴合屏幕最右侧 `right: 0`）。左侧的 2/3 空间完全留白（展示底层视频流）。
    - **快捷平铺卡片（Inline SVGs）**：控制中心卡片网格中必须使用高精度 Inline SVG 图标：
      - *Screensaver (屏幕保护)*: 带波浪线条的方框图标；
      - *Inputs (信号源)*: 框线内带向左指向箭头的输入标识；
      - *Display (图像)*: 带圆心校正点的显示器图标；
      - *Sound (声音)*: 标准扬声器与双波纹声波；
      - *Wi-Fi (无线)*: 渐宽的三层无线信号波束；
      - *Bluetooth (蓝牙)*: 经典蓝牙交叉双折线；
      - *Accessibility (辅助功能)*: 伸展肢体的对称小人。
  - **系统设置菜单（如从控制中心点击 Display/Settings 进入的设置界面）**：从控制中心跳转后，设置菜单界面必须展示在屏幕的**左侧**（贴合屏幕左边缘 `left: 0`，高度垂直铺满屏幕 `height: 100vh`，宽度限制在屏幕的 1/3 或 `360px` 左右，右侧配备效果预览卡片，剩余右侧空间留白展示底层视频流）。**严禁**在设置菜单上使用靠右贴边，也**严禁**生成任何上下有边距、悬浮在屏幕中央的卡片盒子。
    - **杜比视界标志 (Dolby Vision Logo)**：设置菜单头部使用高精度矢量 SVG 绘制（双镜像 "D" 字母相切对立布局 + 纯白 "DOLBY" / "VISION" 文字）。
    - **叶子画面效果对比 (Leaf Preview Card)**：使用专门的高保真绿色树叶电视标定图片（在 React 中引入 relative imports 中的 `tv_preview_leaf.png`），配合下方黑色面板描述，生成 100% 还原 Google TV 风格的对比卡片。

- **【海外区 (Overseas - Fire TV)】横向快捷设置**：
  - 一级分类采用顶部或中部横向排列的图标卡片行（Icons Row），选中后按 Enter 进入下级的垂直属性列表。


### 4. 屏幕分辨率自适应 (Aspect Ratio & Scale Adaptation)
...
### 5. 级联导航路径完整实现与全面交互 (Full Path Menu Navigation & Complete Interaction Flow)
- **从控制中心（Control Center）起航（硬性入口门禁）**：生成的原型 Demo 必须还原从 **“控制中心 (Control Center) / 快捷设置面板 (Quick Settings)”** 开始的完整交互流。当原型页面加载时，**默认首屏必须呈现控制中心悬浮窗/面板**。用户使用 D-pad 选中控制中心内的“全部设置 (All Settings)”（国内为快捷面板中的“设置”/“全部设置”；海外为 Quick Settings 右上角的“Settings”齿轮图标）并点击确认，才能切换/滑入主设置菜单界面。这应当作为两区原型生成时的最外层交互门禁。
- **从一级菜单起航**：在进入主设置菜单后，必须展示从 **“系统设置一级主菜单”**（例如：Picture, Sound, Network, System 等）开始的完整交互流。
- **逐级聚焦与进入（不允许少交互）**：用户使用遥控器 D-pad 物理按键进行导航，必须能够从一级菜单的“图像 (Picture)”聚焦、点击进入二级子菜单、聚焦到特定级联（如其他设置/健康护眼），点击进入三级或四级菜单，直至到达用户核心修改的“命中项”（例如低蓝光模式）。**从控制中心入口、到第一级菜单，再到最后到达的设置项位置，所有交互路径上的操作必须全面实现，绝不允许遗漏或跳过任何级联点击或聚焦操作**。
- **逐级返回闭环**：必须实现完整的返回（Escape / Back 键）回溯逻辑。按返回键时，菜单焦点应倒退回上一层级的触发按钮上，直至退回一级菜单、退回控制中心并关闭，提供完整的链路导航体验。
- **两区全部生效**：上述控制中心入口交互流与返回闭环，**必须在中国区原型和海外区原型中全部严格生效**。

### 6. 菜单层级完全还原与静态占位约束 (Menu Tree Complete Reconstruction & Sibling Completeness)
- **从一级到最终项完整还原（无遗漏）**：设置菜单在生成时，**从第一级到最终到达的菜单项层级必须全部完整还原，绝不允许只渲染其中某几个层级或跳跃式渲染**。以“图像 (Picture)”菜单为例，**必须渲染出数据库中所有的二级菜单功能，包括“高级设置/Advanced Settings”之上或之下的所有同级项（例如“图效模式/Picture Mode”、对比度、亮度等），绝对不能缺失这些设置项**。
- **展示全部同级菜单与相应控制控件**：为了保证原型的视觉绝对真实，生成的菜单面板中不能只显示孤零零的命中节点。必须根据当前区域（国内/海外）的菜单树数据库，在各级面板中渲染出该层级下的**所有真实同级菜单项 (Sibling Items)**。
- **控制控件完整渲染**：在渲染同级菜单项时，**必须根据其功能属性完整渲染出对应的控制控件，例如开关型（Toggle）必须渲染出开关组件、数值/进度调节型（Slider）必须渲染出数值与进度条组件、子入口（Entry）必须渲染出右侧箭头或状态值等**。严禁直接渲染成纯文本。
- **命中路径交互，非命中路径静态占位**：
  - 只有命中的路径及设置项（例如：`Picture > Eye Health Protection > Blue Light Filter`）需要具备完整的二级/三级滑入、按钮点击或数值调节的**全交互**。
  - 其他未命中的同级项（例如：`Brightness` 亮度, `Color` 色彩, `Sound` 声音 等）**也必须渲染出来，且必须在其右侧/结构上渲染出真实的开关、Bar条或入口图标，并支持遥控器聚焦 (Focusable) 以实现光标移动**。点击或调整这些同级项上的控件可以没有任何实际交互状态响应（具体调用组件的效果可以无效/No-op），但视觉上必须完整且丰富。
  - **两区全部生效**：菜单层级还原、同级项控制控件渲染和聚焦逻辑，**必须在中国区原型和海外区原型中全部严格生效**。
