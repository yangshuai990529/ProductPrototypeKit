...
- **命中路径交互，非命中路径静态占位**：
  ...
  - 其他未命中的同级项（例如：`Brightness` 亮度, `Color` 色彩, `Sound` 声音 等）**也必须渲染出来，并支持遥控器聚焦 (Focusable) 以实现光标移动**，点击它们对应的开关、入口、调节Bar条等可以没有任何交互响应（即具体调用组件的效果可以无效/No-op），但必须保证菜单界面的完整填充度。
  - **两区全部生效**：菜单层级还原、同级项完整渲染和聚焦逻辑，**必须在中国区原型和海外区原型中全部严格生效**。
