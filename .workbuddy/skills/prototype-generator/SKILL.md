...
- **1/3 宽度侧边抽屉**：所有的设置面板必须设计为“侧边抽屉（Side Drawer / Overlay Panel）”样式，通常靠右（或靠左）侧对齐，其余的 2/3 空间完全留白，用于展示底层的视频流。
- **高度铺满与贴边对齐**：侧边栏面板必须**高度垂直铺满整个屏幕（即 `height: 100vh` 或 `1080px`，`top: 0; bottom: 0;`）**，并且牢牢**贴合在屏幕边缘（优先靠最右边贴齐，如 `right: 0;`）**。**严禁**生成任何上下有边距、悬浮在屏幕中央、形似卡片盒子的独立悬浮弹窗。它必须像真实电视设置菜单一样贴边且贯穿整个垂直高度。
