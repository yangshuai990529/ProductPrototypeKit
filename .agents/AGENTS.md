...
- **IMPORTANT (Full Path Menu Reconstruction & Full Interaction)**:
  - **Start from Control Center (控制中心)**: The prototype interaction **MUST start from the Control Center (控制中心) by default**. When the page loads, the Control Center overlay (Quick Settings panel) must be displayed first. The user must focus and click the "全部设置" (All Settings) or "Settings" gear icon to transition into the main settings menu.
  - **Complete Sibling Feature Rendering (Zero Omission)**: The generated settings menu, from the first level (e.g. System Settings root) to the final target menu item level, **MUST be fully reconstructed (including all sibling items at each level, such as '图效模式'/'Picture Mode' and other options above '高级设置'/'Advanced Settings')**. Do not omit any features or parent entries.
  - **Control Component Visual Rendering**: Sibling items at each level must be rendered and focusable, and **MUST visually render their corresponding control components (such as toggle switches, slider bars, numerical values, or sub-menu arrows/entry indicators) based on their functional types**, rather than rendering them as plain text. The controls themselves can be dummy non-functional elements.
  - **End-to-End Unbroken D-pad Interactions**: The entire selected path from the Control Center, to the Settings root, and down to the final target setting item **MUST have complete, unbroken end-to-end interactions** (D-pad focus movement, selection, enter, back/escape). No shortcuts or missing interaction steps are allowed.
